...
cdef void fast_graph_compute(
        ...
        np.ndarray[float] result
        ):
    #Input gate
    cdef np.ndarray[float] input_gate_out = expit(np.dot(input_v, w_inpgate) + np.dot(output, w_rec_inpgate) + np.dot(memory, w_mem_inpgate) + w_input_gate_bias)

    #Input processing
    cdef np.ndarray[float] block_input_out = expit(np.dot(input_v, w_inp) + np.dot(output, w_rec_inp) + w_block_input_bias)

    #Gate the Block Input and compute the final input out
    cdef np.ndarray[float] input_out = np.multiply(input_gate_out, block_input_out)

    #Read Gate
    cdef np.ndarray[float] read_gate_out = expit(np.dot(input_v, w_readgate) + np.dot(output, w_rec_readgate) + np.dot(memory, w_mem_readgate) + w_readgate_bias)

    #Compute hidden activation - processing hidden output for this iteration of net run
    cdef np.ndarray[float] hidden_act = np.multiply(read_gate_out, memory) + input_out

    #Write gate (memory cell)
    cdef np.ndarray[float] write_gate_out = expit(np.dot(input_v, w_writegate)+ np.dot(output, w_rec_writegate) + np.dot(memory, w_mem_writegate) + w_writegate_bias)
...
class Drone_Default:
    ...
    def graph_compute(self, input_v, memory): #Feedforwards the input and computes the forward pass of the network
        ...
        result = np.zeros(1)
        fast_graph_compute( input_v, memory, self.output, self.w_block_input_bias, self.w_hid_out, self.w_inp, self.w_inpgate, self.w_input_gate_bias, self.w_mem_inpgate, self.w_mem_readgate, self.w_mem_writegate, self.w_readgate, self.w_readgate_bias, self.w_rec_inp, self.w_rec_inpgate, self.w_rec_readgate, self.w_rec_writegate, self.w_writegate, self.w_writegate_bias, self.params.output_activation, result)
